...
' Constantes del Protocolo Serie
...
Public Const EV_CTS = &H8   'Cambió el estado de Preparado para enviar (CTS)
Public Const EV_RLSD = &H20 'Cambió el estado de Petición de línea para enviar datos (RLSD)
...
Public udtPorts(1 To MAX_PORTS) As COMM_PORT
'-------------------------------------------------------------------------------
...
'
' Returns:
'   Error String - Last serial port error string.
'-------------------------------------------------------------------------------
Public Function CommGetError(ByRef strMessage As String) As Long
...
Public Function CommWait(intPortID As Integer, evento As Long) As Long
Dim lpOverlapped As OVERLAPPED

    'SetCommMask udtPorts(intPortID).lngHandle, evento
    CommWait = WaitCommEvent(udtPorts(intPortID).lngHandle, evento, lpOverlapped)
End Function

Public Function CommSetMask(intPortID As Integer, evento As Long) As Long

    CommSetMask = SetCommMask(udtPorts(intPortID).lngHandle, evento)
    'CommWait = WaitCommEvent(udtPorts(intPortID).lngHandle, evento, lpOverlapped)
End Function

Public Function CommPurge(intPortID As Integer, evento As Long) As Long
    CommPurge = PurgeComm(udtPorts(intPortID).lngHandle, evento)
End Function

Public Function DupDLE(cmd) As String
Dim i As Integer
Dim r As String
    
    For i = 1 To Len(cmd) Step 2
        If Mid$(cmd, i, 2) = "10" Then
            r = r & "1010"
        Else
            r = r & Mid$(cmd, i, 2)
        End If
...
Public Function RepDLE(cmd) As String
Dim i As Integer
Dim r As String
    ...
    i = 1
    While i < Len(r)
        If Mid$(r, i, 4) = "1010" Then
            r = Mid$(r, 1, i + 1) & Mid$(r, i + 4)
        End If
        i = i + 2
    Wend
